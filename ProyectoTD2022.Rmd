...
En primer lugar determinamos la posicion de tanto los datos cuyo valor es infinito como 
```{r}
...
En este caso los vamos a obviar
```{r}
df%>%filter(!Valores==Inf)
```
Ahora vamos a ver si existen valores `NA`
```{r}
is.na(df$Nivel_Sonoro) %>% sum()
aggr(df, prop = FALSE, combined = TRUE, numbers = TRUE, sortVars = TRUE, sortCombs = TRUE)
```
Estos eran los unicos registros que contenian datos erroneos
```{r}
